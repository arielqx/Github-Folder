...
Load the "CommQuest2023_Larger.csv" dataset using the `read_csv()` command and assign it to a variable named "comm_data." 
...
# Loading "CommQuest2023.csv" dataset
...
```{r, eval=TRUE,echo=TRUE}
# selecting column "date", "channel" and "message"
df <- select(comm_data,
       date,
       channel,
       message)
...
```{r, eval=TRUE,echo=TRUE}
#Create new df ("df_1")using filter command 
df_1 <- df %>% 
...
```{r, eval=TRUE,echo=TRUE}
#Using arrange command to arrange "comm_data" df in ascending order based on "date"  
df_2 <- arrange(comm_data,date)
head(df_2)
```
<br>
...
```{r, eval=TRUE,echo=TRUE}
# Applying "distinct" command to find unique senders in "comm_data" df 
comm_data %>% 
  distinct(sender)
```
<br>
...
```{r, eval=TRUE,echo=TRUE}
library(dplyr)
comm_data_new <- comm_data %>% mutate(sentiment_label = case_when(
...
```{r, eval=TRUE,echo=TRUE}
library(dplyr)
comm_data_NEW1 <- comm_data %>% 
  mutate(length = nchar(message), product = sentiment * length) %>%
  arrange(desc(product)) %>%
            select(date,channel,sender,message,length,product)
head(comm_data_NEW1)
```
<br>
...
*Firstly, there are multiple variables in one column.*
For the dataset to be tidy, each variable should have its own column. Hence the first column including "Employment Status" and "Age" as well as "Family status" should be split into three columns instead to make it tidy. 

*Secondly, there is inconsistency in the units of measurement.*
For instance in the column "Percent", there are both percentage values as well as numerical values. This mixes up the units of measurements since 20% is not necessarily = to 20. 
