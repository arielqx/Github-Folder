...
## Note: colon is only for consecutive indices
...
## Note: use c for non-consecutive indices
...
## Accessing elements where x is less than 10 
...
```{r,warning=TRUE,message=TRUE,eval=TRUE,echo=TRUE}
# Retrieve the element named type
## Returns the element without the $name
my_pie[["type"]]
...
loans <- loans_full_schema %>% #%>% is the pipe operator
  select(paid_total, term, interest_rate,
         annual_income,paid_late_fees,debt_to_income)
## selecting only the variables of interest using select()
